...
.inv-img{
	height: 40px; width:181px; color: #ff0000; float:right; margin-bottom: 180px; padding-right: 5px; margin-right: -55px;
}
.inv-para{
	font-size: 13px; float: right; font-weight: 700; margin-top: 40px; margin-right: -20px;
}
.bal-style{
	margin-right: 90px; margin-top: -70px;		
}
.inven-style{
	margin-right: 90px; margin-top: -20px;
}
.bal-new-sty{
	margin-right: 90px; margin-top: -5px;
}
.col-inv{
...
.inv-autho{
	margin-right: 300px; float:right; padding-top: 5px;
}
.inv-img-autho{
...
.inv-aside{
	margin-right: 90px; padding-top: 180px;
}
.solox-sty-wid{
	width:300px;
}
.pay-method-inv{
	color: #ff0000; border: 1px solid lightgray; padding: 8px;
}

</style>
<header>
			<h1 class="background-title-caps">{{trans('custom.invoices.title-caps')}}</h1>

			<h5 class="parafontweight"><b>{{trans('custom.invoices.invoice-to')}}</b></h5>
			...
			<address contenteditable>
				...
				<p class="parawidth">{{$invoice->customer->address}}</p><br>
				<p class="parafontweight">{{trans('custom.common.phone')}} {{$invoice->customer->phone1}}</p>
				@if(! empty( $invoice->customer->email ) )
				<p class="parafontweight">{{trans('custom.common.email')}} {{$invoice->customer->email}}</p>
				@endif
			</address>
			<span class="inv-span">
			<?php
            ...
            ?>
			<img alt="" src="{{asset( 'uploads/settings/' . $logo )}}" class="inv-img">
			<br><p class="inv-para" >{{getSetting('site_title', 'site_settings', trans('global.global_title'))}}</p>
			<br>
			<p class="inv-para">{{getSetting('site_address', 'site_settings')}}</p></span>
		</header>
		<article>
			...
			<table class="inventory inven-style" >
				<thead>
					<tr>
			...
			<table class="balance bal-new-sty">
				<tr>
 ...
 <div class="column" >
    <header>
			<address solox  class="solox-sty-wid">
				...
				@if(! empty( $invoice->invoice_notes ) )
	                ...
	                <p class="pay-method-inv">{!!$invoice->invoice_notes!!}</p>
	            @endif

			</address>
		</header>
  </div>
  <div class="column col-inv">
    <header>
			<address>
				<p class="inv-flort">@lang('custom.invoices.authorized-person')</p><br>
				<?php
				...
				<span class="inv-autho">
					<img alt="" class="inv-img-autho" src="{{asset( 'uploads/settings/' . $authorized_sign )}}">
				</span>
				@endif
				@if( ! empty( $authorized_person ) )
				<span class="inv-autho-per">({{$authorized_person}})</span>
				@endif
				@if( ! empty( $authorized_designation ) )
				<br><span class="inv-autho-per">{{$authorized_designation}}</span>
				@endif
			</address>
		</header>
</div>
		<aside class="inv-aside">
			<h1 ><span contented	itable></span></h1>
