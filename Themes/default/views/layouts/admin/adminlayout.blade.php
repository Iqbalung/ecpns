	...
	<div id="wrapper" class="{{$class}}">
			...
			<ul class="nav navbar-right top-nav">
				...
				<li class="dropdown profile-menu">
					...
					<ul class="dropdown-menu">
						...
						<li>
							<a href="{{URL_FCM_NOTIFICATION}}">
								<span>{{ getPhrase('send_push_notification') }}</span>
								</a>
						</li>
		...
		<aside class="left-sidebar">			<div class="collapse navbar-collapse navbar-ex1-collapse">				<ul class="nav navbar-nav side-nav">					<li {{ isActive($active_class, 'dashboard') }}>
					...
					<ul id="lms" class="collapse sidemenu-dropdown">
						...
						<li {{ isActive($active_class, 'resumes_templates') }} >
						<a href="{{URL_RESUME_TEMPLATES}}" ><i class="fa fa-file-image-o" aria-hidden="true"></i>
					{{ getPhrase('resume_templates') }} </a>
					...
					<li {{ isActive($active_class, 'pages') }} >
						<a href="{{URL_PAGES}}" ><i class="fa fa-book" aria-hidden="true"></i>
					{{ getPhrase('pages') }} </a>
					...
					<li {{ isActive($active_class, 'blogs') }} >
						<a href="{{URL_BLOGS}}" ><i class="fa fa-comment-o" ></i> {{ getPhrase('blogs') }} </a>
					</li>


					<li {{ isActive($active_class, 'faqs') }} >

					<a data-toggle="collapse" data-target="#faqs"><i class="fa fa-question" aria-hidden="true"></i>
					{{ getPhrase('faqs') }} </a>
					...
					<a data-toggle="collapse" data-target="#master_settings" href="{{URL_MASTERSETTINGS_SETTINGS}}"><i class="icon-settings" ></i>
					...
					<ul id="master_settings" class="collapse sidemenu-dropdown">
							...
							@endif
							@if(checkRole(getUserGrade(1)) && 'individual' != validityType() )
							<li><a href="{{URL_PACKAGES}}"> <i class="icon-settings"></i> {{ getPhrase('packages') }}</a></li>
							@endif
					...
					<li {{ isActive($active_class, 'themes') }} >
						<a href="{{URL_THEMES_LIST}}" ><i class="fa fa-th-large" ></i>
					{{ getPhrase('themes') }} </a>
