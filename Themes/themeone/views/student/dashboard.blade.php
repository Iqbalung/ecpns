    ...
    <div id="page-wrapper">
        <div class="container-fluid">
            ...
            @if (session()->has('exam_id') && $quiz_data)
                ...
                @if ($is_purchased && $quiz_data->is_paid == 1)
                    <div class="alert alert-success">
                        <strong>{{ ucwords($quiz_data->title) }}</strong> &nbsp;&nbsp; <a onclick="startMyExam()"
                            href="javascript:void(0);"
                            class="btn btn-primary btn-sm">{{ getPhrase('click_here_to_take_exam') }}</a>
                    </div>
            ...
            @if (session()->has('series_quiz_slug') && $series_exam_link)
                <div class="alert alert-success">
                    <strong>{{ ucwords($series_quiz_data->title) }}</strong> &nbsp;&nbsp; <a onclick="startMySeriesExam()"
                        href="javascript:void(0);"
                        class="btn btn-primary btn-sm">{{ getPhrase('click_here_to_take_exam') }}</a>
                </div>
            @endif
                    ...
                    <div class="media state-media box-ws">
                        <div class="media-left">
                            <a href="{{ URL_STUDENT_EXAM_CATEGORIES }}">
                                <div class="state-icn bg-icon-info"><i class="fa fa-list-alt"></i></div>
                            </a>
                        </div>
                    ...
                    <div class="media state-media box-ws">
                        <div class="media-left">
                            <a href="{{ URL_STUDENT_LMS_CATEGORIES }}">
                                <div class="state-icn bg-icon-purple"><i class="fa fa-tv"></i></div>
                            </a>
                        </div>
                        <div class="media-body">
...
@section('footer_scripts')
    @include('common.chart', [$chart_data, 'ids' => $ids]);
    ...
    <script>
        window.onload = function() {
            ...
            if (series_exam_link) {
                ...
                // console.log("{{ URL_PAYMENTS_CHECKOUT . 'exam/' }}" + pay_exam_slug);
