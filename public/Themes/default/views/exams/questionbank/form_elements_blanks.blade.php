...
<fieldset class="form-group" data-ng-repeat="i in range(total_correct_answers) track by $index" ng-if="total_correct_answers > 0">
        ...
        <textarea name="correct_answers[]" id="option_@{{ $index }}" class="form-control" placeholder="Answer @{{ $index+1 }}" ng-model="correct_answers[$index].answer"  min="1"  required="true" ckeditor></textarea>
        @else
